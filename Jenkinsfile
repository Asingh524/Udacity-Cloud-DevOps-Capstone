...
'''
    ...
    stage('Build Container') {
      steps {
        ...
        sh '''version=$(cut -d= -f2 app_version.txt );docker build --tag=asblogapi:$version .
'''
        sh '''docker image ls
'''
      }
...
docker tag asblogapi:$version 004302033888.dkr.ecr.us-east-2.amazonaws.com/asblog-repo:$version'''
       sh 'version=$(cut -d= -f2 app_version.txt );$(aws ecr get-login --no-include-email --region us-east-2);docker push 004302033888.dkr.ecr.us-east-2.amazonaws.com/asblog-repo:$version'
      }
    }
    stage('check Active Env') {
      steps {
        ...
        sh '''isblue=$(kubectl get deployments | grep asblog |grep blue|wc -l);
if [ $isblue -eq "1" ]
then
  ...
  echo "blue">activeEnv.txt;
   ...
   stage('Waiting for completion and running a test endpoint') {
      steps {
        ...
        sh '''DEPLOYMENTNAME=asblog-deployment-$(cat inactiveEnv.txt);
...
while [[ "$READY" != "True" ]]; do
    ...
    stage('Delete old App and Test Service') {
      steps {
        ...
        sh '#kubectl delete service  asblogapi-service-test;#kubectl delete deployment asblog-deployment-$(cat activeEnv)'
      }
    }
  }
}